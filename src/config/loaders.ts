...
import { LevelRequirement, Ability, Idler, SudokuPuzzle } from '@/types';

export function loadLevelRequirements(): LevelRequirement[] {
  return levelsData.levelRequirements;
}

export function loadAbilities(): Ability[] {
  return abilitiesData.abilities;
}

export function loadIdlers(): Idler[] {
  return idlersData.idlers;
}

export async function loadSudokuBoard(boardNumber: number): Promise<SudokuPuzzle> {
  try {
...
export function getRandomBoardNumber(): number {
