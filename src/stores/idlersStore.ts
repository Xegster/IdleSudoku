...
import { IdlerProgress } from '@/types';
import { usePlayerStore } from './playerStore';

interface IdlerStore {
  idlerProgress: Map<string, IdlerProgress>;
  loadIdlers: () => Promise<void>;
  saveIdler: (idlerId: string, level: number, lastUpdateTime: number) => Promise<void>;
  upgradeIdler: (idlerId: string) => void;
  updateIdlerProgress: (idlerId: string, lastUpdateTime: number) => void;
  calculateProduction: (idlerId: string) => number;
  getUnlockedIdlers: () => string[];
}

export const useIdlersStore = create<IdlerStore>((set, get) => ({
  idlerProgress: new Map(),

  loadIdlers: async () => {
    try {
      const db = getDatabase();
      const results = await db.getAllAsync<IdlerProgress>(
        'SELECT * FROM idlerProgress'
      );
      
      const progress = new Map<string, IdlerProgress>();
  ...
  saveIdler: async (idlerId: string, level: number, lastUpdateTime: number) => {
    try {
      const db = getDatabase();
      await db.runAsync(
  ...
  updateIdlerProgress: (idlerId: string, lastUpdateTime: number) => {
    const progress = get().idlerProgress.get(idlerId);
    if (!progress) return;
  ...
  calculateProduction: (idlerId: string) => {
    const idlerConfig = getIdlers().find(i => i.id === idlerId);
